...
int main() {                  // V 1.02
	...
	int trovato;    // variabile di stato della ricerca: 0 --> non trovato
	//                                   1 --> trovato
	int continuare; // variabile di stato della ricerca: 0 --> non continuare la ricerca
	//                                   1 --> continuare la ricerca
	char x[LS];     // cognome da ricercare
	int i;          // indice per la ricerca
	int li;         // limite inferiore ricerca dicotomica
	int ls;         // limite superiore ricerca sequenziale
	int m;          // valore medio della ricerca dicotomica
	...
	do { // Inizio ciclo di ripetizione del programma
		...
		switch(scelta) {
			...
			case 4: {
				system("CLS");
				if(caricato==0) {
					printf("\n\n Ricerca voti impossibile\n\n Caricamento dati non effettuato ! ! !\n");
					getch();
				} else {
					if(ordinato==1) {
						printf("\n\n Immettere il cognome dell'alunno da ricercare --> ");
						fflush(stdin);
						gets(x);
						nc=strlen(x);
						for(i=0; i<nc; i++) {
						...
						if(errore==0) {
							...
							do {
								...
								if(c==0) {
									...
									if(li>ls) continuare=0;
								}
							} while(continuare==1);
							if(trovato==1) {
								gotoxy(2,2);
								printf(" Cognome      Nome       Rel Ita Sto Ing Mat Inf Tps Sis Tel EFi Cnd");
								gotoxy(2,3);
								printf("-------------------------------------------------------------------------");
								gotoxy(3,5);
								puts(Tabellone[m].co);
								...
								for(iv=0; iv<NV; iv++) {
									gotoxy(26+4*iv,4+it);
									printf(" %2d",Tabellone[m].vf[iv]);
								}
								getch();
							} else {
								printf("Nessun alunno trovato, assicurarsi di aver immesso il cognome correttamente ! ! !");
								getch();
							}
						}
					} else {

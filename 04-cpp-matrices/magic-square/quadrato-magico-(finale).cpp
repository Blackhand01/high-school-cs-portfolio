...
int main()
{
	...
	int corn, l, i, x, y, Quadrato[DIM][DIM];
	char RispUscita;
	...
	do{
		...
		for(riga=0; riga<l; riga++)             //inizializzazione matrice, tutti le posizioni le metto a 0
		{
			for(col=0; col<l; col++) 
			{
				Quadrato[riga][col]=0;
		...
		for(i=1; i<(l*l); i++)				//finchè i è minore dell'area del Quadrato (nella matrice ci saranno l*l numeri) il ciclo si ripete
		{
			if(i%l==0)						//se il numero corrente è un multiplo del lato del quadrato
			{
		...
		Quadrato[riga][col]=i+1; 			//inserisco il numero successivo nella posizione stabilita all'interno della matrice 
		}
...
int cortemp=15;         //tempo di formazione della cornice
